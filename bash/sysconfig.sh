...
function error-message {
  echo "Error: $@" >&2 # error for all arguments in command line, sent to standard error log
}
...
function cleanup {
  rm /tmp/sysinfo.* /tmp/memoryinfo.* /tmp/businfo.* /tmp/cpuinfo.* /tmp/sysreport.* # all tmp files found in the script
  logger -t `basename "$0"` -i -p user.info -s "Cleaning up and aborting"
  exit 1
}

trap cleanup SIGINT   # SIGINT issued is the user sends an interrupt signal Ctrl C
