...
# Task 1
hostname=$(hostname)
interface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') # command to isolate interface
lanAddress=$(ip a s $interface|awk '/inet /{gsub(/\/.*/,"");print $2}') # used interface variable to simplify this script
...
# Task 2
routerAddress=$(route -n | grep -w UG | awk $'{print $2}')
