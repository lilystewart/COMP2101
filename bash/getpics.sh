...
rm -rf ~/public_html/pics ~/public_html/tarfiles
mkdir -p ~/public_html/pics || (echo "Failed to make a new pics directory" && exit 1)
mkdir -p ~/public_html/tarfiles || (echo "Failed to make new tarfiles directory" && exit 1)
...
wget -q -O ~/public_html/pics/pics.tgz http://zonzorp.net/pics.tgz && tar -zxf ~/public_html/pics/pics.tgz -C ~/public_html/tarfiles && rm ~/public_html/pics/pics.tgz
...
EOF
# Made a report for the tarfiles directory within public_html as a separate folder was created for the tar download.
test -d ~/public_html/tarfiles && cat <<EOF
Found $(find ~/public_html/tarfiles -type f|wc -l) files in the public_html/tarfiles directory.
The ~/public_html/tarfiles directory uses $(du -sh ~/public_html/tarfiles|awk '{print $1}') space on the disk.
EOF
