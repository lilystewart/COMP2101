...
for interface in "${interfaces[@]}"; do   # for each interface in interface array, do the following (if -v included in command line include verbose):
  ...
  network_address=$(ip route list dev $interface scope link | awk '{print $1'} | sed 's+/24+''+g')
  ...
  grep -q $network_address /etc/networks || echo "network_$interface" "$network_address"|sudo tee -a /etc/networks
